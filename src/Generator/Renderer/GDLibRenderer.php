...
/**
 * Raster renderer that paints the QR matrix using the GD extension.
 *
 * This renderer targets PNG, GIF, and JPEG output. It allocates a temporary GD
 * image, fills the background, paints finder eyes and data modules as polygons,
 * and finally serializes the image into the requested format.
...
final class GDLibRenderer implements RendererInterface
{
    ...
    public function __construct(
        ...
        if (!extension_loaded('gd') || !function_exists('gd_info')) {
            throw RuntimeException::withMessage('You need to install the GD extension to use this back end');
        }
        ...
        if ($this->fill->hasGradientFill()) {
            throw InvalidArgumentException::withMessage('GDLibRenderer does not support gradients');
        }
    }

    /**
     * Render the QR matrix into an encoded GD-backed image blob.
     *
     * The method removes finder patterns from the payload matrix first because
     * this renderer paints eyes separately with their dedicated fill settings.
     *
    ...
    /**
     * Allocate a fresh GD image and paint the background fill.
     */
    private function newImage(): void
    {
        ...
        if ($img === false) {
            throw RuntimeException::withMessage('Failed to create image of that size');
        }
    ...
    /**
     * Paint finder eyes and remaining dark modules onto the current image.
     */
    ...
    /**
     * Paint one finder eye using its internal and external fill colors.
     */
    ...
    /**
     * Serialize the current GD image into the configured output format.
     */
    ...
    /**
     * Resolve and cache a GD palette entry for the supplied renderer color.
     *
     * Alpha-wrapped colors are converted to GD's inverted `0..127`
     * transparency scale before allocation.
     */
    private function getColor(ColorInterface $color): int
    {
        ...
        if (!isset($this->colors[$colorKey])) {
            ...
            if ($colorId === false) {
                throw RuntimeException::withMessage('Failed to create color: #'.$colorKey);
            }
