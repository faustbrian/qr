...
use Cline\Qr\Generator\Common\BitMatrix;

test('get set', function (): void {
    $matrix = new BitMatrix(33);
    $this->assertSame(33, $matrix->getHeight());
...
test('rectangular matrix', function (): void {
...
test('rectangular set region', function (): void {
    ...
    for ($y = 0; $y < 240; ++$y) {
        for ($x = 0; $x < 320; ++$x) {
            $this->assertSame($y >= 22 && $y < 34 && $x >= 105 && $x < 185, $matrix->get($x, $y));
        }
    }
});

test('get row', function (): void {
