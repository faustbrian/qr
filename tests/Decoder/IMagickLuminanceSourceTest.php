...
it('exports and crops luminance data', function (): void {
    ...
    try {
        ...
        expect($source->getMatrix())->toBeArray();

        $cropped = $source->crop(0, 0, 1, 1);

        expect($cropped)->toBeInstanceOf(IMagickLuminanceSource::class)
            ->and($cropped->getMatrix())->toBe([0 => $source->getMatrix()[0]]);
    } finally {
